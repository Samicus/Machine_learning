...
class DeepQNetwork(nn.Module):
    def __init__(self, num_states, num_actions):
        ...
        #layer_1_width = config["model_params"]["layer_1_width"]
        #layer_2_width = config["model_params"]["layer_2_width"]
        #layer_3_width = config["model_params"]["layer_3_width"]
        layer_1_width = 128
        layer_2_width = 128
        layer_3_width = 128
...
class DeepDoubleQNetwork(object):
    def __init__(self, device, num_states, config):
        self.device = device
        ...
        self.online_model = DeepQNetwork(self.num_states, self.num_actions, config).to(
            device=self.device
        )
        self.offline_model = DeepQNetwork(self.num_states, self.num_actions, config).to(
            device=self.device
        )
